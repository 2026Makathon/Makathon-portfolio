# 2026 메커톤 포트폴리오 소개
***
## 기획팀
* **메이플스토리 월드 기반 교육/심리 케어 플랫폼 - 원병선**
  * 메이플 스토리 월드를 활용한 '학습 무동기(슬럼프)'극복 프로그램

## 개발팀
* **Beat Maple - 원병선**
  * 메이플 스토리 기반 리듬 게임
* **PacMan - 박형규**
  * 메이플 스토리 요소를 가미한 PacMan게임
* **슈팅게임 - 박영준**
  * 메이플 스토리 디자인을 담은 슈팅 미니 게임
  * https://github.com/pyj-0313/Maple-Shooting-game

### 필독 : .exe 파일 실행 방법
https://www.oracle.com/kr/java/technologies/downloads/#jdk26-windows <br>
링크에 들어가서 각 os에 맞는 최신 버전 x64 Installer를 다운로드하면 됩니다.
이후 각 .exe 파일을 실행해주세요.
***
## 디자인팀
* **강유정**
  * 전투 밖에서도 이어질 게임 캐릭터들의 생활을 볼 수 있는 '몬스터 헌터 기업 사내 시스템'을 상상하며 제작한 게임 UI/UX 컨셉 프로젝트 제작
* **정예지**
  * 일러스트와 삼면도, 스탠딩 이미지로 구성된 포트폴리오를 제작

 
